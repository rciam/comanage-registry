<?php
/**
 * COmanage Registry Assurance Index View
 *
 ...
 */
  ...
  $params = array('title' => _txt('ct.assurances.pl'));
...
<div class="table-container">
  <table id="assurances">
    <thead>
      <tr>
        <th><?php print $this->Paginator->sort('value', _txt('fd.assurance.value')); ?></th>
    ...
    <tbody>
    <?php $i = 0; ?>
    <?php foreach ($assurances as $c): ?>
    <tr class="line<?php print ($i % 2)+1; ?>">
      <td>
        <?php
          print $this->Html->link($c['Assurance']['value'],
                                 array('controller' => 'assurances',
                                       'action' => ($permissions['edit'] ? 'edit' : ($permissions['view'] ? 'view' : '')), $c['Assurance']['id']));
        ?>
      </td>
      <td>
        <?php
          if(!empty($c['Assurance']['org_identity_id']))
          {
      ...
      <td>
        <?php
          if(!empty($c['Assurance']['co_person_id']))
          {
        ...
        <?php
          if($permissions['edit']) {
            print $this->Html->link('Edit',
                array('controller' => 'assurances', 'action' => 'edit', $c['Assurance']['id']),
          ...
          if($permissions['delete']) {
            print '<button type="button" class="deletebutton" title="' . _txt('op.delete')
              ...
              . filter_var(_jtxt($c['Assurance']['value']),FILTER_SANITIZE_STRING)  // dialog body text replacement strings
              . '\']);">'
              . _txt('op.delete')
              . '</button>';
          }
